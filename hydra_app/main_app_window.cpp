...
static void Init(std::shared_ptr<IHRRenderDriver> pDriverImpl)
{
  ...
  // std::shared_ptr<IHRRenderDriver> pDriverImpl = nullptr;
  // 
  // if(g_input.enableMLT)
  //   pDriverImpl = std::shared_ptr<IHRRenderDriver>(CreateDriverRTE(L"", g_width, g_height, g_input.inDeviceId, 16384)); // GPU_MLT_ENABLED_AT_START
  // else 
  //   pDriverImpl = std::shared_ptr<IHRRenderDriver>(CreateDriverRTE(L"", g_width, g_height, g_input.inDeviceId, 0));
