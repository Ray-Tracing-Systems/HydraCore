...
float3  IntegratorShadowPT::PathTrace(float3 ray_pos, float3 ray_dir, MisData misPrev, int a_currDepth, uint flags)
{
  ...
  if (lightOffset >= 0)
  {
    ...
    const float3 brdfVal    = materialEval(pHitMaterial, &sc, (EVAL_FLAG_DEFAULT), /* global data --> */ m_pGlobals, m_texStorage, m_texStorageAux, &ptlCopy).brdf; // a_shadingTexture
...
float3 IntegratorMISPT::PathTrace(float3 ray_pos, float3 ray_dir, MisData misPrev, int a_currDepth, uint flags)
{
  ...
  float3 resColor = explicitColor + cosTheta * bxdfVal * PathTrace(nextRay_pos, nextRay_dir, currMis, a_currDepth + 1, flags);  // --*(1.0 / (1.0 - pabsorb));
  return clamp3(resColor, 0.0F, 1e6F);
}
