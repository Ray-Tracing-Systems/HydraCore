...
typedef struct GlobalRenderDataT
{
  ...
  float m_essGGXSample2Table[4096];
  float m_essTranspGGXTable[4096];


} EngineGlobals;

#ifndef OCL_COMPILER
static inline void InitEngineGlobals(EngineGlobals* a_pGlobals, const float* a_ggxData, const float* a_transpGgxData)
{
  memset(a_pGlobals, 0, sizeof(EngineGlobals));
  for(int i=0;i<QMC_VARS_NUM;i++)
    a_pGlobals->rmQMC[i] = -1;

  memcpy(a_pGlobals->m_essGGXSample2Table, a_ggxData,      sizeof(float) * 4096);
  memcpy(a_pGlobals->m_essTranspGGXTable, a_transpGgxData, sizeof(float) * 4096);
}
#endif
...
/**
 \brief return size of random numbers vector for kmlt. In 32 bit words/floats. In fact  
*/
static inline int kmltBlobSize(__global const EngineGlobals* a_globals) 
{
  return KMLT_HEAD_SIZE + a_globals->varsI[HRT_KMLT_OR_QMC_LGT_BOUNCES]*KMLT_PER_LIGHT + a_globals->varsI[HRT_KMLT_OR_QMC_MAT_BOUNCES]*KMLT_PER_MATERIAL;
}

/**
 \brief return offset in random numbers vector to light numbers
*/
static inline int kmltLightOffset(const int a_depth)
{
  return KMLT_HEAD_SIZE + a_depth*KMLT_PER_LIGHT;
}

/**
 \brief return offset in random numbers vector to light numbers
*/
static inline int kmltMaterialOffset(const int a_depth, __global const EngineGlobals* a_globals)
{
  return KMLT_HEAD_SIZE + a_globals->varsI[HRT_KMLT_OR_QMC_LGT_BOUNCES]*KMLT_PER_LIGHT + a_depth*KMLT_PER_MATERIAL;
}
