...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  inline __device__ __host__ float clamp(float u, float a, float b)    { return fmin(fmax(a, u), b); }
  ...
  typedef image2d_t texture2d_t;


  typedef ushort4 half4;
  ...
  ID_CALL int  atomic_add(int*  pData,  int a_val) { return atomicAdd(pData, a_val); }
  ID_CALL uint atomic_add(uint* pData, uint a_val) { return atomicAdd(pData, a_val); }

  typedef unsigned short half;
  ID_CALL void vstore_half(float data, size_t offset, __global half *p) { p[offset] = __float2half_rn(data); }

  #define ALIGN_S(x) __align__(x)

  #define ENABLE_OPACITY_TEX 1
  #define SHADOW_TRACE_COLORED_SHADOWS 1
  //#define NEXT_BOUNCE_RR 1
  #define ENABLE_BLINN 1
  //#define RAYTR_THREAD_COMPACTION 1

  IDH_CALL float sign(float a) { return (a > 0.0f) ? 1.0f : -1.0f; }

  ID_CALL void atomic_addf(__global float *source, const float operand)
  { 
...
IDH_CALL float2 ProjectedPixelSize(float dist, float2 FOV, float w, float h)
{
  return make_float2(FOV.x / w, FOV.y / h)*dist;
}

IDH_CALL float pixelSimilarity(float4 data1, float4 data2, float MADXDIFF)
{
  ...
  float normalDiff = sqrt(1.0f - (dist / MANXDIFF));
  float depthDiff = sqrt(1.0f - fabs(d1 - d2) / MADXDIFF);

  return normalDiff*depthDiff;
}

IDH_CALL float pixelsDiff(float4 data1, float4 data2, float MADXDIFF)
{
  const float MANXDIFF = 0.25f;
  ...
  if (length(n1 - n2) <= MANXDIFF && fabs(d1 - d2) <= MADXDIFF)
    return 0.0f;
  else
    return 1.0f;
}
