...
static inline Lite_Hit IntersectAllPrimitivesInLeaf(const float3 a_ray_pos, const float3 a_ray_dir,
                                                    ...
                                                    Lite_Hit a_result,
                                                     ...
                                                     const int a_instId)
{
  const double3 ray_pos = double3(a_ray_pos);
  const double3 ray_dir = double3(a_ray_dir);
  ...
  for (int triAddress = triAddressStart; triAddress < triAddressEnd; triAddress += NUM_FETCHES_TRI)
  {
    ...
    const double3 A_pos = double3(to_float3(data1));
    const double3 B_pos = double3(to_float3(data2));
    const double3 C_pos = double3(to_float3(data3));
    ...
    if(det != 0.0)
    {
      ...
      //  std::cout << "u,v = " << u << "," << v << " : uv failed!" << std::endl;
                                            ...
                                            __global const float4* a_bvh, __global const float4* a_tris, const int a_targetInstId)
{
  ...
  while (top >= 0)
  {
    ...
    if (top >= 0 && instDeep == 1)
    {
      if (a_targetInstId == -1 || a_targetInstId == instId)
      {
...
static inline SurfaceHit surfaceEvalLS(const float3 a_rpos, const float3 a_rdir, const Lite_Hit hit, __global const PlainMesh* mesh)
{
  ...
  const bool badTangent = (!isfinite(surfHit.biTangent.x) || !isfinite(surfHit.biTangent.y) || !isfinite(surfHit.biTangent.z));

  if(fabs(fabs(dot(surfHit.normal,surfHit.tangent)) - 1.0f) < 1e-4f || badTangent)  // broken tangent space inside model, recalculate it in some way
