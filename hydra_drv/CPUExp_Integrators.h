...
/**
\brief Mix/Blend of IBPT and MMLT via per-pixel mask

*/
class IntegratorThreeWayBlendMMLT : public IntegratorCommon 
{
public:

  IntegratorThreeWayBlendMMLT(int w, int h, EngineGlobals* a_pGlobals) : IntegratorCommon(w, h, a_pGlobals, 0)
  {
    m_imageDL.resize(w*h);
    m_imageGI.resize(w*h);
    m_imageGI2.resize(w*h);
    m_hdrDL  = &m_imageDL[0];
    m_hdrGI  = &m_imageGI[0];
    m_hdrGI2 = &m_imageGI2[0];

    m_pIntegratorMMLT = new IntegratorMMLT(w, h, a_pGlobals, m_hdrGI2);

    memset(m_hdrDL,  0, w*h * sizeof(float) * 4);
    memset(m_hdrGI,  0, w*h * sizeof(float) * 4);
    memset(m_hdrGI2, 0, w*h * sizeof(float) * 4);

    m_avgB = 1.0f;
  }

  void SetSceneGeomPtrs(SceneGeomPointers a_data)     override { m_geom          = a_data;              m_pIntegratorMMLT->SetSceneGeomPtrs(a_data); }
  void SetMaterialStoragePtr(const float4* a_data)    override { m_matStorage    = a_data;              m_pIntegratorMMLT->SetMaterialStoragePtr(a_data); }
  void SetTexturesStoragePtr(const float4* a_data)    override { m_texStorage    = (const int4*)a_data; m_pIntegratorMMLT->SetTexturesStoragePtr(a_data); }
  void SetPdfStoragePtr     (const float4* a_data)    override { m_pdfStorage    = a_data;              m_pIntegratorMMLT->SetPdfStoragePtr(a_data);  }
  void SetTexturesStorageAuxPtr(const float4* a_data) override { m_texStorageAux = (const int4*)a_data; m_pIntegratorMMLT->SetTexturesStorageAuxPtr(a_data); }
...
protected:
  ...
  std::vector<float>  m_noiseGI;
  std::vector<float4> m_imageDL; ///< direct light
  std::vector<float4> m_imageGI; ///< indirect light - i.e. Global Illumination.
  std::vector<float4> m_imageGI2;

  float4* m_hdrDL;               ///< @always = &m_imageDL[0]
  float4* m_hdrGI;               ///< @always = &m_imageGI[0]
  float4* m_hdrGI2;              ///< @always = &m_imageGI2[0]

  IntegratorMMLT* m_pIntegratorMMLT;
  float m_avgB;
};
