...
void IntegratorLT::ConnectEye(SurfaceHit a_hit, float3 ray_dir, float3 a_accColor, int a_currBounce)
{
  ...
  if(a_currBounce > 0) // if 0, this is light surface
  {
    ...
    BxDFResult matRes = materialEval(pHitMaterial, &sc, (EVAL_FLAG_FWD_DIR | EVAL_FLAG_APPLY_GLOSS_SIG), /* global data --> */ m_pGlobals, m_texStorage, m_texStorageAux, &m_ptlDummy);
