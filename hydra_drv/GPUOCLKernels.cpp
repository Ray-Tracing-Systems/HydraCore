...
void GPUOCLLayer::runKernel_UpdateRevAccGTermAndSavePrev(cl_mem a_rayFlags, cl_mem a_rpos, cl_mem a_rdir, int a_currBounce, size_t a_size)
{
  cl_kernel kernX      = m_progs.material.kernel("UpdateRevAccGTermAndSavePrev");
  ...
  CHECK_CL(clSetKernelArg(kernX, 0, sizeof(cl_mem), (void*)&a_rayFlags));
  ...
  CHECK_CL(clSetKernelArg(kernX, 4, sizeof(cl_mem), (void*)&m_rays.hitNormUncompressed));
  CHECK_CL(clSetKernelArg(kernX, 5, sizeof(cl_mem), (void*)&m_rays.pathMisDataPrev));

  CHECK_CL(clSetKernelArg(kernX, 6, sizeof(cl_mem), (void*)&m_rays.accPdf));
  CHECK_CL(clSetKernelArg(kernX, 7, sizeof(cl_mem), (void*)&m_rays.oldColor)); // when 3-Way PT pass run, it use unused 'oldColor' to store prevData that is a copy of m_rays.accPdf
  CHECK_CL(clSetKernelArg(kernX, 8, sizeof(cl_mem), (void*)&m_rays.oldFlags)); // when 3-Way PT pass run, it use unused 'oldFlags' to store pdfCamA as single float (sizeof(int) == sizeof(float)) 
  CHECK_CL(clSetKernelArg(kernX, 9, sizeof(cl_mem), (void*)&m_scene.allGlobsData));

  CHECK_CL(clSetKernelArg(kernX, 10, sizeof(cl_float), (void*)&mLightSubPathCount)); // a_mLightSubPathCount
  CHECK_CL(clSetKernelArg(kernX, 11, sizeof(cl_int),   (void*)&currBounce));         // a_currDepth
  CHECK_CL(clSetKernelArg(kernX, 12, sizeof(cl_int),   (void*)&isize));              // int   iNumElements
...
void GPUOCLLayer::ShadePass(cl_mem a_rpos, cl_mem a_rdir, cl_mem a_outColor, size_t a_size, bool a_measureTime)
{
  ...
  else
  {
    ...
    CHECK_CL(clSetKernelArg(kernZ, 14, sizeof(cl_mem), (void*)&m_scene.storageTex));
    CHECK_CL(clSetKernelArg(kernZ, 15, sizeof(cl_mem), (void*)&m_scene.storageTexAux));
    CHECK_CL(clSetKernelArg(kernZ, 16, sizeof(cl_mem), (void*)&m_scene.storageMat));

    CHECK_CL(clSetKernelArg(kernZ, 17, sizeof(cl_int), (void*)&isize));
    CHECK_CL(clSetKernelArg(kernZ, 18, sizeof(cl_mem), (void*)&m_scene.allGlobsData));
