...
void GPUOCLLayer::runKernel_ComputeAO2(cl_mem outCompressedAO, size_t a_size) // #TODO: implement this! Both Up and down !!!
{
  ...
  CHECK_CL(clSetKernelArg(kernAO, 6, sizeof(cl_mem), (void*)&m_rays.shadowRayPos));
  CHECK_CL(clSetKernelArg(kernAO, 7, sizeof(cl_mem), (void*)&m_rays.shadowRayDir));
  //CHECK_CL(clSetKernelArg(kernAO, 8, sizeof(cl_mem), (void*)&m_rays.hitInstanceId)); // #TODO: find some tmp data, for example 'm_rays.hitProcTexData'  
  ...
  // (3) calc shadows
  //
  runKernel_ShadowTraceAO(m_rays.rayFlags, m_rays.shadowRayPos, m_rays.shadowRayDir, m_rays.lshadow, 4*a_size); 
...
void GPUOCLLayer::runKernel_HitEnvOrLight(cl_mem a_rayFlags, cl_mem a_rpos, cl_mem a_rdir, cl_mem a_outColor, int a_currBounce, size_t a_size)
{
  ...
  CHECK_CL(clSetKernelArg(kernX, 12, sizeof(cl_mem), (void*)&m_rays.pathShadow8B));        // 

  CHECK_CL(clSetKernelArg(kernX, 13, sizeof(cl_mem), (void*)&m_rays.pathMisDataPrev));
  CHECK_CL(clSetKernelArg(kernX, 14, sizeof(cl_mem), (void*)&m_rays.accPdf));
  CHECK_CL(clSetKernelArg(kernX, 15, sizeof(cl_mem), (void*)&m_rays.oldColor));            // when 3-Way PT pass run, it use unused 'oldColor' to store prevData that is a copy of m_rays.accPdf
  CHECK_CL(clSetKernelArg(kernX, 16, sizeof(cl_mem), (void*)&m_rays.oldFlags));            // when 3-Way PT pass run, it use unused 'oldFlags' to store pdfCamA as single float (sizeof(int) == sizeof(float)) 
  ...
  CHECK_CL(clSetKernelArg(kernX, 24, sizeof(cl_float), (void*)&mLightSubPathCount)); // a_mLightSubPathCount
  CHECK_CL(clSetKernelArg(kernX, 25, sizeof(cl_int),   (void*)&currBounce));         // a_currDepth
...
void GPUOCLLayer::runKernel_ShadowTraceAO(cl_mem a_rayFlags, cl_mem a_rpos, cl_mem a_rdir, cl_mem a_outShadow, size_t a_size)
{
