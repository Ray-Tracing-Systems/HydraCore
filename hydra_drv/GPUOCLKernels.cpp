...
void GPUOCLLayer::runKernel_ComputeHit(cl_mem a_rpos, cl_mem a_rdir, size_t a_size, bool a_doNotEvaluateProcTex)
{
  ...
  if (m_rays.hitProcTexData != nullptr)
  {
    ...
    CHECK_CL(clSetKernelArg(kernProcT, 8, sizeof(cl_mem), (void*)&m_scene.matrices));

    CHECK_CL(clSetKernelArg(kernProcT, 9, sizeof(cl_mem), (void*)&m_rays.hitProcTexData));
    
    CHECK_CL(clSetKernelArg(kernProcT,10, sizeof(cl_mem), (void*)&m_scene.storageTex));
    CHECK_CL(clSetKernelArg(kernProcT,11, sizeof(cl_mem), (void*)&m_scene.storageMat));
    CHECK_CL(clSetKernelArg(kernProcT,12, sizeof(cl_mem), (void*)&m_scene.allGlobsData));
    CHECK_CL(clSetKernelArg(kernProcT,13, sizeof(cl_int), (void*)&isize));
...
void GPUOCLLayer::runKernel_HitEnvOrLight(cl_mem a_rayFlags, cl_mem a_rpos, cl_mem a_rdir, cl_mem a_outColor, int a_currBounce, size_t a_size)
{
  ...
  CHECK_CL(clSetKernelArg(kernX, 12, sizeof(cl_mem), (void*)&m_rays.oldRayDir));           // when PT: use oldRayDir to store emission color
  CHECK_CL(clSetKernelArg(kernX, 13, sizeof(cl_mem), (void*)&m_rays.pathShadow8B));        // 

  CHECK_CL(clSetKernelArg(kernX, 14, sizeof(cl_mem), (void*)&m_rays.pathMisDataPrev));
  CHECK_CL(clSetKernelArg(kernX, 15, sizeof(cl_mem), (void*)&m_rays.accPdf));
  CHECK_CL(clSetKernelArg(kernX, 16, sizeof(cl_mem), (void*)&m_rays.oldColor));            // when 3-Way PT pass run, it use unused 'oldColor' to store prevData that is a copy of m_rays.accPdf
  CHECK_CL(clSetKernelArg(kernX, 17, sizeof(cl_mem), (void*)&m_rays.oldFlags));            // when 3-Way PT pass run, it use unused 'oldFlags' to store pdfCamA as single float (sizeof(int) == sizeof(float)) 
  ...
  CHECK_CL(clSetKernelArg(kernX, 22, sizeof(cl_mem), (void*)&m_scene.allGlobsData));
  CHECK_CL(clSetKernelArg(kernX, 23, sizeof(cl_mem), (void*)&m_scene.instLightInst));
  CHECK_CL(clSetKernelArg(kernX, 24, sizeof(cl_mem), (void*)&m_rays.hits));

  CHECK_CL(clSetKernelArg(kernX, 25, sizeof(cl_float), (void*)&mLightSubPathCount)); // a_mLightSubPathCount
  CHECK_CL(clSetKernelArg(kernX, 26, sizeof(cl_int),   (void*)&currBounce));         // a_currDepth
  CHECK_CL(clSetKernelArg(kernX, 27, sizeof(cl_int),   (void*)&isize));
...
void GPUOCLLayer::runKernel_NextTransparentBounce(cl_mem a_rpos, cl_mem a_rdir, cl_mem a_thoroughput, size_t a_size)
{
  ...
  CHECK_CL(clSetKernelArg(kernX, 6, sizeof(cl_mem), (void*)&a_thoroughput)); // a_thoroughput
  CHECK_CL(clSetKernelArg(kernX, 7, sizeof(cl_mem), (void*)&m_scene.storageMat));
