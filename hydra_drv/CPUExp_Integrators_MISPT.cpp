...
void IntegratorMISPTLoop2Adapt::AdaptFromLuminance(int2& a_pos, RandomGen& a_gen)
{
  ...
  float nextLum              = Luminance(float3(m_summColors[nextIndex].x, m_summColors[nextIndex].y, m_summColors[nextIndex].z));
  ...
  float& a_max)
{
  ...
  for (int offsetY = -a_sizeLocalWindow; offsetY <= a_sizeLocalWindow; offsetY++)
  {
    for (int offsetX = -a_sizeLocalWindow; offsetX <= a_sizeLocalWindow; offsetX++)
    {
      ...
      const float val = Luminance(float3(m_summColors[index].x, m_summColors[index].y, m_summColors[index].z));
...
float IntegratorMISPTLoop2Adapt::MathExp2(const int2 a_pos, const int2 a_nextPos, const int a_sizeLocalWindow)
{
  ...
  for (int offsetY = -a_sizeLocalWindow; offsetY <= a_sizeLocalWindow; offsetY++)
  {
    for (int offsetX = -a_sizeLocalWindow; offsetX <= a_sizeLocalWindow; offsetX++)
    {
      ...
      const float currVal    =  Luminance(float3(m_summColors[currIndex].x, m_summColors[currIndex].y, m_summColors[currIndex].z));
      const float nextVal    =  Luminance(float3(m_summColors[nextIndex].x, m_summColors[nextIndex].y, m_summColors[nextIndex].z));
